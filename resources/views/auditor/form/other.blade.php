...
<script>
    $(document).ready(function () {
        ...
        function loadForm(tabParent, idQuestion, idActive, responseAns) {
            ...
            $.ajax({
                url: "{{ route('form.getChecklistForm', encrypt($id)) }}",
                method: 'GET',
                data: {
                    tabParent: tabParent,
                                    ...
                                    ${response.tabLists.map(item => `
                                        <li class="nav-item">
                                            <a class="nav-link ${item.parent_point_checklist === response.tabParentAct ? 'active' : ''}" id="tabBtn" dataTab="${item.parent_point_checklist}" dataQuest="${item.firstIdQuestion}">
                                                <button type="button" class="lmt btn btn-sm ${item.rejected === 1 ? 'btn-danger' : item.isFullFilled === 1 ? 'btn-success' : 'btn-light'}">
                                                                    ...
                                                                    <div class="col-8">
                                                                        ${response.options.map((item, index) => `
                                                                            <div class="form-check">
                                                                                <input class="form-check-input" type="radio" id="option_${index}" name="options" value="${item.meta_name}" ${item.meta_name === response.question.response ? 'checked' : ''} ${response.question.approve === 3 ? 'disabled' : ''}>
        ...
        function uploadResponseFile(idCheckJar, idActive, responseFile, callback) {
            if (responseFile) {
                ...
                $.ajax({
                    url: "{{ route('form.storeChecklistFile') }}",
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
        ...
        $(document).on('click', '#finishBtn', function (e) { 
            ...
            formData.append('responseFile', responseFile);
            $.ajax({
                url: "{{ route('form.finishChecklist') }}",
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,

...
class MstAssignChecklistController extends Controller
{
    ...
    //     $jaringan = MstPeriodeChecklists::select('type')
    //     ->where('mst_periode_checklists.id', $period->id)
    //     ->join('mst_dealers', 'mst_periode_checklists.id_branch', '=', 'mst_dealers.id')->first();

    //         $typecount = MstMapChecklists::select('type_checklist as type')
    //         ->where('mst_mapchecklists.type_jaringan', $jaringan->type)
    ...
    //         ->orderBy('mst_parent_checklists.id')
    //         ->get();
    

    //     $data = DataTables::of($typecount)
    ...
    public function type(Request $request, $id, $type)
    {
        ...
        //     ->get();
        $jaringan = MstPeriodeChecklists::leftjoin('mst_dealers', 'mst_periode_checklists.id_branch', 'mst_dealers.id')
        ->where('mst_periode_checklists.id', $id)
        ->first()->type;
        // dd($jaringan);
        $checklists = MstParentChecklists::where('type_checklist', $type)
            ->whereNotIn('mst_parent_checklists.id', function($query) use ($jaringan) {
                $query->select('id_parent_checklist')
                    ->from('mst_mapchecklists')
                    ->where('type_jaringan', $jaringan);
            })
            ->get();
            // dd($checklists);
